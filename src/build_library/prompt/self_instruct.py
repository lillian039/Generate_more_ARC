...
You will be given several concepts, you should utilize the given concepts, make similar concepts, which means only edit some of its component or combine the given concepts to create new concepts.
"""
...
def get_content_similar(descriptions, k_new):
    content_str = "Seed concepts share some key ideas, new concepts should also follow that key ideas. New concepts must use given seed concepts, which means having the same core concept while only edit some of its component or combine the given concepts to create new concepts. You should make sure each concept executable. The concept should be refined and no more than one sentence.\n\n"
    for i, descript in enumerate(descriptions):
        content_str += f"Seed concept {i + 1}: {descript}\n"
    content_str += "\n"
    length = len(descriptions)
    for i in range(k_new):
        content_str += f"Concept {length + i + 1}: [new concept]\n"
